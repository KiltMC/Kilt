...
object ItemBlockRenderTypesRemap {
    internal val CUTOUT_MIPPED = net.minecraftforge.client.ChunkRenderTypeSet.of(RenderType.cutoutMipped())
    internal val SOLID = net.minecraftforge.client.ChunkRenderTypeSet.of(RenderType.solid())
    internal val BLOCK_RENDER_TYPES = Util.make(Object2ObjectOpenHashMap<Holder.Reference<Block>, net.minecraftforge.client.ChunkRenderTypeSet>(ItemBlockRenderTypesAccessor.getTypeByBlock().size, .5F)) {
        it.defaultReturnValue(SOLID)
        ItemBlockRenderTypesAccessor.getTypeByBlock().forEach { entry ->
            it[ForgeRegistries.BLOCKS.getDelegateOrThrow(entry.key)] = net.minecraftforge.client.ChunkRenderTypeSet.of(entry.value)
        }
    }
    ...
    @JvmStatic
    fun getRenderLayers(state: BlockState): net.minecraftforge.client.ChunkRenderTypeSet {
    ...
    fun setRenderLayer(block: Block, predicate: Predicate<RenderType>) {
        setRenderLayer(block, net.minecraftforge.client.ChunkRenderTypeSet.of(RenderType.chunkBufferLayers().filter { predicate.test(it) }))
    }

    @JvmStatic
    @Synchronized
    fun setRenderLayer(block: Block, layers: net.minecraftforge.client.ChunkRenderTypeSet) {
        checkClientLoading()
