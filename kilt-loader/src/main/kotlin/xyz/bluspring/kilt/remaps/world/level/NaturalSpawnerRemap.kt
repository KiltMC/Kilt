        ...
        return if (type == SpawnPlacements.Type.NO_RESTRICTIONS) {
            ...
            when (type) {
                ...
                SpawnPlacements.Type.IN_LAVA -> fluidState.`is`(FluidTags.LAVA)
                    ...
                    if (!(blockState2 as common.extensions.IForgeBlockState).isValidSpawn(levelReader, blockPos3, type, entityType)) {
                        false
                    } else {
                        NaturalSpawner.isValidEmptySpawnBlock(levelReader, blockPos, blockState, fluidState, entityType
                        ) && NaturalSpawner.isValidEmptySpawnBlock(levelReader, blockPos2, levelReader.getBlockState(blockPos2), levelReader.getFluidState(blockPos2), entityType)
                    }
                }
            }
        } else {
