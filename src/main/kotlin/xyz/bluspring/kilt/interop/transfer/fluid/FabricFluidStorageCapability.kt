...
open class FabricFluidStorageCapability(val storage: Storage<FluidVariant>) : IFluidHandler {
    ...
    override fun fill(resource: FluidStack, action: IFluidHandler.FluidAction): Int {
        TransferUtil.getTransaction().use { transaction ->
            val inserted = storage.insert(FluidVariant.of(resource.fluid, resource.tag), resource.amount.toLong(), transaction)
    ...
    override fun drain(resource: FluidStack, action: IFluidHandler.FluidAction): FluidStack {
        TransferUtil.getTransaction().use { transaction ->
            val extracted = storage.extract(FluidVariant.of(resource.fluid, resource.tag), resource.amount.toLong(), transaction)
    ...
    override fun drain(maxDrain: Int, action: IFluidHandler.FluidAction): FluidStack {
        TransferUtil.getTransaction().use { transaction ->
            ...
            for (view in storage.toList()) {
                ...
                selectedStack.amount += totalExtracted.toInt()
