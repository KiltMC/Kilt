        ...
        run {
            ...
            run {
                ...
                ClassTinkerers.addTransformation(geoItemRenderer) { classNode ->
                    // Modifies GeoItemRenderer's <init> to point to BlockEntityWithoutLevelRenderer's <init>()V
                    run {
                        val bewlrClassIm = "net.minecraft.class_756"
                        val bewlrClass = mappingResolver.mapClassName("intermediary", bewlrClassIm).replace(".", "/")
                        val initMethod = classNode.methods.first { it.name == "<init>" && it.desc == "(Lsoftware/bernie/geckolib3/model/AnimatedGeoModel;)V" }

                        initMethod.instructions.forEach { insn ->
                            if (insn is MethodInsnNode && insn.owner == "java/lang/Object" && insn.name == "<init>" && insn.desc == "()V") {
                                insn.owner = bewlrClass
                            }
                        }
                    }
                }
